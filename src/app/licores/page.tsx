...
import licoresMenu from "@/data/licores-menu.json";
...
import { useLicoresCart } from "@/lib/cart/licores-cart";
import { cartCount, cartTotal } from "@/lib/cart/createCartStore";
import { buildLicoresOrderMessage } from "@/lib/cart/messages";
import type { DeliveryType } from "@/lib/cart/delivery";

const world = worlds.licores;

export default function LicoresPage() {
  useRevealWorld();
  const cart = useLicoresCart();
  ...
  async function handleCheckout() {
    ...
    const message = buildLicoresOrderMessage(cart.lines, cart.notes, total, customerName, deliveryType, deliveryAddress);
  ...
  return (
    <>
      ...
      <MenuCatalog world={world} categorias={licoresMenu.categorias} onAddToCart={handleAddToCart} />
